...
public class ControllerViewModel : BindableBase, IDisposable
{
            ...
            while (processedDataCount < maxProcessPerFrame && GlobalValues.TcpClient.ChannelReader.TryRead(out var data))
            {
                ...
                lock (_updateLock)
                {
                    if (data.ChannelId >= 0 && data.ChannelId < _seriesCount)
                    {
                        ...
                        processedDataCount += data.Data.Length;
    ...
    private void ProcessAndUpdateCharts()
    {
        try
        {
            ...
            for (var i = 0; i < _seriesCount; i++)
            {
                var buffer = new List<float>(allPointsCount); // 预分配容量提高性能
                var queue = _channelBuffers[i];

                // 从队列中取出数据直到达到所需点数
                while (queue.Count > 0 && buffer.Count < allPointsCount)
                {
                    var data = queue.Peek();
                    if (buffer.Count + data.Length <= allPointsCount)
                    {
                    ...
                    {
                        // 只取需要的部分
                        var remaining = allPointsCount - buffer.Count;
                        buffer.AddRange(data.Take(remaining));
                        var newArray = data.Skip(remaining).ToArray();
                        queue.Dequeue();
                        if (newArray.Length > 0) queue.Enqueue(newArray);
                    }
                }

                channelData[i] = buffer.ToArray();
                Interlocked.Add(ref PointNumbers[i], -allPointsCount);
            }
    ...
    private static double CalculateAnnotationWidth(string text, double fontSize = 14)
    {
        ...
        var width = text.Length * fontSize * 0.7 + fontSize * 2;
    ...
    public void UpdateCursorResult(LightningChart chart)
    {
        try
        {
            ...
            var xAxisMin = xAxis.Minimum;
            var xAxisMax = xAxis.Maximum;
            ...
            for (var i = 0; i < cursors.Count; i++)
            {
                ...
                foreach (var t in series)
                {
                    ...
                    if (
                        SolveValueAccurate(t, cursors[i].ValueAtXAxis, out var seriesYValue)
                ...
                // 检查光标是否靠近右边界
                var cursorX = cursors[i].ValueAtXAxis;

                // 判断光标是否在图表右侧区域（例如：最后20%区域）
                var isNearRightEdge = cursorX > xAxisMax - xAxisVisible * 0.1;
                ...
                for (var j = 0; j < validAnnotations.Count; j++)
                {
                    ...
                    // 根据文本计算所需的宽度
                    var requiredWidthPx = CalculateAnnotationWidth(annotationText);
            ...
            try
            {
                while (IsRunning)
                    if (FileData.TryDequeue(out var data))
                    {
                        ...
                        // 使用一个文件保存所有通道数据
                        var fullPath = Path.Combine(FolderPath, $"{timestamp}.txt");
        ...
        if (canvas.Children[0] is LightningChart chart)
        {
            ...
            foreach (
                ...
                select new MenuItem
                {
                    ...
                    Command = new DelegateCommand(() => { UpdateTitle(series1); })
                }
            )
            ...
            menu.Items.Add(
                ...
                {
                    Header = "重置缩放",
                    Command = new DelegateCommand(() => { chart.ViewXY.ZoomToFit(); })
                }
            );
    ...
    private void DeleteSeriesCursor(LightningChart chart, int cursorIndex)
    {
        ...
        // 删除该光标对应的注释（如果有）
        var series = chart.ViewXY.SampleDataSeries;
        var annotationStartIndex = cursorIndex * (series.Count + 1);

        // 删除与此光标关联的所有注释（X轴注释和每个系列注释）
        var annotationsToRemove = series.Count + 1;
        for (var i = 0; i < annotationsToRemove; i++)
            // 确保我们不会越界
        ...
        where T : IDisposable
    {
        ...
        while (list.Count > 0)
        {
            ...
            if (item != null) item.Dispose(); // then dispose it.
        }
    }
    ...
    private void InitializeFileWatcher()
    {
        ...
        _fileWatcher.Created += (_, e) =>
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                if (FileNames.Count >= 1000) FileNames.RemoveAt(FileNames.Count - 1); // 移除最旧的一项
